...
#  compile.sh
#  
#  Shellscript para compilar programinhas didaticos em C.
#
#  Versao 2.0
#
#  Created by Edwino Stein on 04/08/14.
#
...
function compile(){
	...
	# Compila o arquivo fonte e pega a saida e o codigo do GCC
	compilerInvocation="gcc -Wall -o ${dirBuild}${target}.o -c ${src}";
...
if [ ! -f "${dirBuild}${fileCommon}.o" ] ; then
	echo -n "Compilando o dependencias comuns \"${fileCommon}.c\"....";

	compile "${dirCommonLib}${fileCommon}.c";
	checkError;
	
fi
dependencies="$dependencies ${dirBuild}${fileCommon}.o";
...
linkerInvocation="gcc -Wall -Werror -o ${dirBuild}${main} ${dependencies} -lm";
